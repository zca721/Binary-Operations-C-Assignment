...
int main() {
    // Left blank so grader tests can go here, sometimes with malloc and
    // creating a lot of arrays cause an overlap of indexes and an array
    // gets stored over a previous pointer to a malloc array when ran and
    // causes an 'abort' error to happen.
    // So, I figured I would put a seperate class with no previous function
    // calls so the tests used by the grader do not cause any 'abort' errors
    // because there is already a lot of malloc arrays being made at runtime.

    printf("Grader Unit Test Cases--------------------------------------\n");
